...
def parse_track(track):
    """Парсит трек из API в формат для БД (БЕЗ ID)"""
...
def insert_into_db(tracks):
    """Вставляет новые треки в базу данных (БЕЗ УКАЗАНИЯ ID)"""
    ...
    try:
        ...
        for track in tracks:
            try:
                # Вставляем БЕЗ ID - пусть база сама назначает
                cur.execute("""
                ...
                print(f"⚠️ Ошибка вставки трека {track.get('artist')} - {track.get('track')}: {e}")
                conn.rollback()
                continue
...
def update_scrobbles_unic():
    ...
    try:
        ...
        # Обновляем уникальные треки используя новые данные
        cur.execute("""
            WITH new_tracks AS (
                SELECT DISTINCT artist, track 
                FROM public.scrobbles 
                WHERE play_date > COALESCE(
                    (SELECT MAX(last_seen) FROM public.scrobbles_unic), 
                    '1970-01-01'
                )
            )
            ...
            SELECT 
                s.artist, 
                s.track, 
                MIN(s.play_date) as first_seen,
                MAX(s.play_date) as last_seen,
                COUNT(*) as play_count
            FROM public.scrobbles s
            JOIN new_tracks nt ON s.artist = nt.artist AND s.track = nt.track
